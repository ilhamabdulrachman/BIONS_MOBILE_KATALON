...
import com.kms.katalon.core.webui.keyword.WebUiBuiltInKeywords as mobile
...
class NetworkChecker {
	...
	def static verifyInternetConnection() {
		...
		def adbCommand = 'adb shell ping -c 1 192.168.19.61'

		try {
			def process = Runtime.getRuntime().exec(adbCommand)
			def output = new StringBuilder()
			new BufferedReader(new InputStreamReader(process.getInputStream())).eachLine { output.append(it) }

			if (output.toString().contains('1 received')) {
				println("Koneksi internet berhasil.")
			} else {
				throw new Exception("Koneksi terputus. Ping gagal.")
			}
		} catch (Exception e) {
			String errorMessage = 'Koneksi internet terputus atau gagal. Menghentikan tes. Error: ' + e.getMessage()
			println(errorMessage)

			// Memberi label kegagalan resmi di Report
			KeywordUtil.markFailed(errorMessage)

			//Error messages di log
			Mobile.comment("PENTING: Tes dihentikan karena masalah koneksi jaringan.")

			// Menghentikan seluruh eksekusi tes
