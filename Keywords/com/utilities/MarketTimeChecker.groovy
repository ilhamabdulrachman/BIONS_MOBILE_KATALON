...
class MarketTimeChecker {
	...
	private static Long findEpoch(byte[] allBytes) {
		...
		String asText = null
		try {
			asText = new String(allBytes, "UTF-8")
		} catch (Exception e) {
			asText = null
		}
		if (asText) {
		...
		for (int i = 0; i + 4 <= allBytes.length; i++) {
			...
			if (v > 946684800L && v < 2556144000L) {
				// between 2000 and ~2050 in seconds
				return v
			}
		}
		...
		for (int i = 0; i + 8 <= allBytes.length; i++) {
			...
			if (v > 946684800000L && v < 2556144000000L) {
				// plausible millis
				return v
			}
		}
